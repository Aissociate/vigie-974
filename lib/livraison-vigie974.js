...
function selectRows(all, zone, n) {
  const inPerimeter = poolDisponible(all);
...
async function livrerLaBase({ email, contacts, zone, exclusif }) {
  ...
  if (exclusif && rows.length) {
    const restant = ajouterVendus(rows.map(cleContact));
