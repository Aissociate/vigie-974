...
const UNIT_EUR     = 50;       // 50 € le contact (standard)
...
function disponibles() {
  return Math.max(0, MAX_CONTACTS - compteVendus());
}
...
function unitAmountCents(qty, exclusif) {
  let cents;
  if (qty >= MAX_CONTACTS)      cents = 3500; // toute la base → -30 %  (35 €)
...
async function handleWebhook(req, res, rawBody) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    try {
      await livrerLaBase({ email, contacts, zone, exclusif });
    } catch (e) {
